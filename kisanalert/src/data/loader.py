...
def _resolve_csv_path(crop: str = None) -> Path:
    """Resolve the raw CSV path based on the current crop in config."""
    c = (crop or config.TARGET_COMMODITY).lower()
    district = config.TARGET_DISTRICT.lower()
    # Try the well-known pattern: {crop}_{district}.csv
    candidate = config.RAW_DIR / f"{c}_{district}.csv"
    if candidate.exists():
        return candidate
    # Fallback to the hardcoded soybean path
    return config.RAW_CSV_PATH


def load_clean_data(csv_path: Path = None) -> pd.DataFrame:
    """
    Public entry point for Phase 1.
    Returns a clean, daily-frequency DataFrame with no nulls.
    Automatically picks the CSV for config.TARGET_COMMODITY when csv_path is not given.
    """
    if csv_path is None:
        csv_path = _resolve_csv_path()
        
    def _process_single_dataset(path: Path):
        d = load_raw(path)
        d = normalise_columns(d)
        d = parse_dates(d)
        d = pick_price_qty_columns(d)
        d = aggregate_multiple_markets(d)
        d = reindex_to_daily(d)
        return d
    ...
    # --- LEAD-LAG ENGINE (Surrounding Merge) ---
    c = config.TARGET_COMMODITY.lower()
    main_district = config.TARGET_DISTRICT.lower()
    surrounding_files = list(config.RAW_DIR.glob(f"{c}_*.csv"))
    surrounding_prices = []
    
    for path in surrounding_files:
        dist_name = path.stem.replace(f"{c}_", "")
        if dist_name == main_district or dist_name == c:
            continue
            
        log.info("Surrounding dataset (%s) found. Merging for Lead-Lag Engine...", dist_name)
        df_surr = _process_single_dataset(path)
        
        # In rare cases, a file might be empty after processing
        if df_surr.empty or "modal_price" not in df_surr.columns:
            continue
            
        col_name = f"{dist_name}_price"
        df_surr = df_surr[['date', 'modal_price']].rename(columns={'modal_price': col_name})
        df = pd.merge(df, df_surr, on='date', how='left')
        df[col_name] = df[col_name].ffill().bfill()
        surrounding_prices.append(col_name)

    if surrounding_prices:
        df['surrounding_price'] = df[surrounding_prices].mean(axis=1)
    else:
        log.warning("No surrounding datasets found. Fallback to modal_price.")
        df['surrounding_price'] = df['modal_price']

    # --- MACRO ENGINE (Historical CBOT / USD/INR) ---
    try:
        from src.data.macro_loader import fetch_macro_data
        log.info("Fetching historical macro data for ML features...")
        df_macro = fetch_macro_data(start_date=config.DATE_START, end_date=config.DATE_END)
        # Merge exactly on date
        df = pd.merge(df, df_macro, on="date", how="left")
        # Ensure columns exist even if yfinance failed
        if "usd_inr" not in df.columns:
            df["usd_inr"] = 83.0
            df["cbot_close"] = 1200.0
